...
class DeleteGroupFlow(private val groupId: UniqueIdentifier, private val notary: Party? = null) : MembershipManagementFlow<SignedTransaction>() {
    ...
    override fun call(): SignedTransaction {
        ...
        val membersWithoutGroup = oldParticipantsMemberships.filter { membership ->
            membership.state.data.identity.cordaIdentity !in (service.getAllBusinessNetworkGroups(networkId) - group).flatMap { it.state.data.participants }
        }
        ...
        syncMembershipsParticipants(networkId, oldParticipantsMemberships, signers, service, notary)
